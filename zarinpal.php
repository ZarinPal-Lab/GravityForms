...
class GFZarinPal_HANNANStd {
    ...
    public static function admin_notice_persian_gf() {
    	...
    	$message = sprintf(__("برای استفاده از درگاه های پرداخت گراویتی فرم نصب بسته فارسی ساز الزامی است . برای نصب فارسی ساز %sکلیک کنید%s.", "gravityformszarinpal"), '<a href="'.admin_url("plugin-install.php?tab=plugin-information&plugin=persian-gravity-forms&TB_iframe=true&width=772&height=884").'">', '</a>');
    ...
    public static function admin_notice_gf_support() {
        $class = 'notice notice-error';
        $message = sprintf(__("درگاه زرین پال نیاز به گراویتی فرم نسخه %s به بالا دارد . برای به روز رسانی به %sصفحه افزونه%s مراجعه نمایید .", "gravityformszarinpal"), self::$min_gravityforms_version, "<a href='plugins.php'>", "</a>");
        printf( '<div class="%1$s"><p>%2$s</p></div>', $class, $message );
    }
	...
	public static function uninstall(){   
		if( ! GFZarinPal_HANNANStd::has_access("gravityforms_zarinpal_uninstall") )
	...
	public static function zarinpal_form_settings_page() {
		...
		<h3>
			...
			<a id="add-new-confirmation" class="add-new-h2" href="<?php echo esc_url( admin_url('admin.php?page=gf_zarinpal&view=edit&fid='. absint(rgget("id")) ) ) ?>"><?php esc_html_e( 'افزودن فید جدید', 'gravityformszarinpal' ) ?></a></span>
            <a class="add-new-h2"  href="admin.php?page=gf_zarinpal&view=stats&id=<?php echo absint(rgget("id")) ?>"><?php _e("نمودار ها", "gravityformszarinpal") ?></a>
        </h3>
	...
	private static function list_page( $arg ){
        ...
        if( ! self::is_gravityforms_supported() ){
            die(sprintf(__("درگاه زرین پال نیاز به گراویتی فرم نسخه %s دارد . برای به روز رسانی به %sصفحه افزونه%s مراجعه نمایید .", "gravityformszarinpal"), self::$min_gravityforms_version, "<a href='plugins.php'>", "</a>"));
        }
        ...
        <div class="wrap">
			...
			<h2>
				...
				if(get_option("gf_zarinpal_configured")){ ?>
					<a class="add-new-h2"  href="admin.php?page=gf_zarinpal&view=edit"><?php _e("افزودن جدید", "gravityformszarinpal") ?></a>
					<?php
				} ?>
            </h2>
            ...
            <form id="confirmation_list_form" method="post">
                ...
                <div class="tablenav">
                    <div class="alignleft actions" style="padding:8px 0 7px 0;">
                        ...
                        ?>
						<a class="button button-primary"  href="admin.php?page=gf_settings&subview=gf_zarinpal"><?php _e('تنظیمات حساب زرین پال', 'gravityformszarinpal') ?></a>
                    </div>
                </div>
                <table class="wp-list-table widefat fixed striped toplevel_page_gf_edit_forms" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" id="active" class="manage-column" style="width:<?php echo $arg != 'per-form' ? '50px' : '20px' ?>" ><?php echo $arg != 'per-form' ? __('وضعیت','gravityformszarinpal') : '' ?></th>
							<th scope="col" class="manage-column" style="width:<?php echo $arg != 'per-form' ? '65px' : '30%' ?>" ><?php _e(" آیدی فید", "gravityformszarinpal") ?></th>
							<?php if ( $arg != 'per-form' ) { ?>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th scope="col" id="active" class="manage-column"><?php echo $arg != 'per-form' ? __('وضعیت','gravityformszarinpal') : '' ?></th>
						...
						<?php }
                            ...
                            foreach($settings as $setting){
                                ...
                                <tr class='author-self status-inherit' valign="top">
									...
									<td><img style="cursor:pointer;" src="<?php echo esc_url( GFCommon::get_base_url() ) ?>/images/active<?php echo intval($setting["is_active"]) ?>.png" alt="<?php echo $setting["is_active"] ? __("درگاه فعال است", "gravityformszarinpal") : __("درگاه غیر فعال است", "gravityformszarinpal");?>" title="<?php echo $setting["is_active"] ? __("درگاه فعال است", "gravityformszarinpal") : __("درگاه غیر فعال است", "gravityformszarinpal");?>" onclick="ToggleActive(this, <?php echo $setting['id'] ?>); " /></td>

									<td><?php echo $setting["id"] ?>
                                        <?php if ( $arg == 'per-form' ) { ?>
                                            <div class="row-actions">
                                                <span class="edit">
                                                    <a title="<?php _e("ویرایش فید", "gravityformszarinpal")?>" href="admin.php?page=gf_zarinpal&view=edit&id=<?php echo $setting["id"] ?>" ><?php _e("ویرایش فید", "gravityformszarinpal") ?></a>
                                                    |
                                                </span>
                                                <span class="trash">
									...
									<td class="column-title">
										...
										<div class="row-actions">
                                            ...
                                            <span class="trash">
                                                <a title="<?php _e("حذف فید", "gravityformszarinpal") ?>" href="javascript: if(confirm('<?php _e("فید حذف شود؟ ", "gravityformszarinpal") ?> <?php _e("\'Cancel\' برای انصراف, \'OK\' برای حذف کردن.", "gravityformszarinpal") ?>')){ DeleteSetting(<?php echo $setting["id"] ?>);}"><?php _e("حذف", "gravityformszarinpal")?></a>
                                                |
                                            </span>
											<span class="view">
                                            ...
                                            <span class="view">
                                                <a title="<?php _e("نمودارهای فرم", "gravityformszarinpal")?>" href="admin.php?page=gf_zarinpal&view=stats&id=<?php echo $setting["form_id"] ?>"><?php _e("نمودارهای فرم", "gravityformszarinpal") ?></a>
                                            </span>
                                        </div>
                                    </td>
									<?php } ?>
        ...
        <script type="text/javascript">
            ...
            function ToggleActive(img, feed_id){
                ...
                mysack.onError = function() { alert('<?php _e("خطای Ajax رخ داده است", "gravityformszarinpal" ) ?>' )};
                mysack.runAJAX();
                return true;
            }
        </script>
	...
	public static function settings_page(){
        ...
        if( rgpost("uninstall") ){	
			...
			if ( isset($_POST["gf_zarinpal_configured"]) )
				update_option("gf_zarinpal_configured", $_POST["gf_zarinpal_configured"]);
			else
				delete_option("gf_zarinpal_configured");
		}
        else{
        ...
        <form action="" method="post">
            ...
            <table class="form-table">
				...
				<tr>
                    <th scope="row"><label for="gf_zarinpal_merchent"><?php _e("فعالسازی", "gravityformszarinpal"); ?></label> </th>
                    <td>
						<input type="checkbox" name="gf_zarinpal_configured" id="gf_zarinpal_configured" <?php echo get_option("gf_zarinpal_configured") ? "checked='checked'" : ""?>/>
						<label class="inline" for="gf_zarinpal_configured"><?php _e("بله", "gravityformszarinpal"); ?></label>
                    </td>
                </tr>
				
				
                <tr>
                    <th scope="row"><label for="gf_zarinpal_server"><?php _e("کشور سرور", "gravityformszarinpal"); ?></label> </th>
                    <td>
                        
						<input type="radio" name="gf_zarinpal_server" value="Iran" <?php echo rgar($settings, 'server') == "Iran" ? "checked='checked'" : "" ?>/>
                        <?php _e("ایران", "gravityformszarinpal"); ?>
                       
						<input type="radio" name="gf_zarinpal_server" value="German" <?php echo rgar($settings, 'server') != "Iran" ? "checked='checked'" : "" ?>/>
                ...
                <tr>
                    ...
                    <td>
                        <input style="width:350px;text-align:left;direction:ltr !important" type="text" id="gf_zarinpal_merchent" name="gf_zarinpal_merchent" value="<?php echo esc_attr(rgar($settings, 'merchent')) ?>" />
                    </td>
                </tr>
				...
				<tr>
                    ...
                    <td>
                        <input style="width:350px;" type="text" id="gf_zarinpal_gname" name="gf_zarinpal_gname" value="<?php echo $gateway_title; ?>" />
                    </td>
                </tr>
        ...
        <form action="" method="post">
			...
			if( GFCommon::current_user_can_any("gravityforms_zarinpal_uninstall") ){
			...
			?>
				...
				<div class="delete-alert alert_red">
					...
					<div class="gf_delete_notice"><?php _e("تذکر : بعد از غیرفعالسازی تمامی اطلاعات مربوط به زرین پال حذف خواهد شد", "gravityformszarinpal") ?></div>
		...
		<div class="wrap gforms_edit_form gf_browser_gecko">
			...
			<?php if(!empty($_get_form_id)) { ?>
				<span class="gf_admin_page_subtitle">
					<span class="gf_admin_page_formid"><?php echo sprintf(__("فید: %s", "gravityformszarinpal"), $id) ?></span>
					<span class="gf_admin_page_formname"><?php echo sprintf( __("فرم: %s", "gravityformszarinpal"), $form_name ) ?></span>
				</span>
			<?php } ?>
			
			</h2>
			<a class="button add-new-h2" href="admin.php?page=gf_settings&subview=gf_zarinpal" style="margin:8px 9px;"><?php _e("تنظیمات حساب زرین پال", "gravityformszarinpal") ?></a>
			...
			if( ! rgempty("gf_zarinpal_submit")){
                ...
                ?>
                <div class="updated fade" style="padding:6px"><?php echo sprintf(__("فید به روز شد . %sبازگشت به لیست%s.", "gravityformszarinpal"), "<a href='?page=gf_zarinpal'>", "</a>") ?></div>
                <?php
			}
							...
							if ( RG_CURRENT_VIEW != 'entry' ) { ?>
								<select name="form_switcher" id="form_switcher" onchange="GF_SwitchForm(jQuery(this).val());">
									...
									<?php foreach ( $feeds as $feed ) { 
										...
										<option value="<?php echo $feed["id"] ?>" <?php echo $selected ?> ><?php echo sprintf(__( 'فرم: %s (فید: %s)', 'gravityformszarinpal' ) , $feed["form_title"], $feed["id"] ) ?></option>
									<?php } ?>
								</select>
								<?php
							}
					...
					<div class="gform_tab_content" id="tab_<?php echo !empty($current_tab) ? $current_tab : '' ?>">
						<div id="form_settings" class="gform_panel gform_panel_form_settings">
							...
							<form method="post" action=""  id="gform_form_settings" >	
								...
								<table class="form-table gforms_form_settings" cellspacing="0" cellpadding="0">
									<tbody>
										...
										<tr>
											...
											<td>
												<select id="gf_zarinpal_form" name="gf_zarinpal_form" onchange="GF_SwitchFid(jQuery(this).val());">
												...
												<img src="<?php echo esc_url( GFCommon::get_base_url() ) ?>/images/spinner.gif" id="zarinpal_wait" style="display: none;"/>
											</td>
										</tr>
								...
								<div id="gf_zarinpal_invalid_product_form" class="gf_zarinpal_invalid_form" style="background-color:#FFDFDF; margin-top:4px; margin-bottom:6px;padding:18px; border:1px dotted #C89797;">
								...
								<?php } else { ?>
								<table class="form-table gforms_form_settings" id="zarinpal_field_group" <?php echo empty($_get_form_id) ? "style='display:none;'" : "" ?> cellspacing="0" cellpadding="0">
									<tbody>
										...
										<tr>
											...
											<td>
												<input type="checkbox" name="gf_zarinpal_type" id="gf_zarinpal_type_subscription" value="subscription" <?php echo rgar($config['meta'], 'type') == "subscription" ? "checked='checked'" : "" ?>/>
										...
										<tr>
											...
											<td>
												<input type="text" name="gf_zarinpal_desc_pm" id="gf_zarinpal_desc_pm" class="fieldwidth-1" value="<?php echo rgar($config["meta"],"desc_pm") ?>"/>
												<span class="description"><?php _e("شورت کد ها : {form_id} , {form_title} , {entry_id}", "gravityformszarinpal"); ?></span>
											</td>
										</tr>
										...
										<tr <?php echo $display_post_fields ? "" : "style='display:none;'" ?>>
											...
											<td>
												<select id="gf_zarinpal_update_action1" name="gf_zarinpal_update_action1">
													<option value="default" <?php echo rgar($config["meta"],"update_post_action1") == "default" ? "selected='selected'" : ""?>><?php _e("وضعیت پیشفرض فرم", "gravityformszarinpal") ?></option>
										...
										<tr <?php echo $display_post_fields ? "" : "style='display:none;'" ?>>
											...
											<td>
												<select id="gf_zarinpal_update_action2" name="gf_zarinpal_update_action2">
                                                    <option value="dont"    <?php echo rgar($config["meta"],"update_post_action2") == "dont" ? "selected='selected'" : ""?>><?php _e("عدم ایجاد پست", "gravityformszarinpal") ?></option>
													<option value="default" <?php echo rgar($config["meta"],"update_post_action2") == "default" ? "selected='selected'" : ""?>><?php _e("وضعیت پیشفرض فرم", "gravityformszarinpal") ?></option>
											...
											<td>
												...
												foreach ( $confirmations as $confirmation ) {	?>
													<li class="gf_zarinpal_confirmation">
														<input id="gf_zarinpal_conf_1_<?php echo $confirmation['id'] ?>" name="gf_zarinpal_conf_1[]" type="checkbox" class="confirmation_checkbox" value="<?php echo $confirmation['id'] ?>" <?php checked( true, in_array( $confirmation['id'], $selected_confirmations ) ) ?> />
														<label for="gf_zarinpal_conf_1_<?php echo $confirmation['id'] ?>" class="inline" for="gf_zarinpal_selected_confirmations"><?php echo $confirmation['name']; ?></label>
													</li>
												<?php } ?>
											</td>
										</tr>	
										...
										<tr id="gf_zarinpal_confirmations_2" <?php echo !isset($form["confirmations"]) ? "style='display:none;'" : "" ?>>
											...
											<td>
												...
												foreach ( $confirmations as $confirmation ) {	?>
													<li class="gf_zarinpal_confirmation">
														<input id="gf_zarinpal_conf_2_<?php echo $confirmation['id'] ?>" name="gf_zarinpal_conf_2[]" type="checkbox" class="confirmation_checkbox" value="<?php echo $confirmation['id'] ?>" <?php checked( true, in_array( $confirmation['id'], $selected_confirmations ) ) ?> />
														<label for="gf_zarinpal_conf_2_<?php echo $confirmation['id'] ?>" class="inline" for="gf_zarinpal_selected_confirmations"><?php echo $confirmation['name']; ?></label>
													</li>
												<?php } ?>
											</td>
										</tr>	
										...
										<tr id="gf_zarinpal_confirmations_3" <?php echo !isset($form["confirmations"]) ? "style='display:none;'" : "" ?>>
											...
											<td>
												...
												foreach ( $confirmations as $confirmation ) {	?>
													<li class="gf_zarinpal_confirmation">
														<input id="gf_zarinpal_conf_3_<?php echo $confirmation['id'] ?>" name="gf_zarinpal_conf_3[]" type="checkbox" class="confirmation_checkbox" value="<?php echo $confirmation['id'] ?>" <?php checked( true, in_array( $confirmation['id'], $selected_confirmations ) ) ?> />
														<label for="gf_zarinpal_conf_3_<?php echo $confirmation['id'] ?>" class="inline" for="gf_zarinpal_selected_confirmations"><?php echo $confirmation['name']; ?></label>
													</li>
												<?php } ?>
											</td>
										</tr>	
											...
											<td>
												...
												foreach ( $notifications as $notification ) {	?>
													<li class="gf_zarinpal_notification">
														<input id="gf_zarinpal_notif_1_<?php echo $notification['id'] ?>" name="gf_zarinpal_notif_1[]" type="checkbox" class="notification_checkbox" value="<?php echo $notification['id'] ?>" <?php checked( true, in_array( $notification['id'], $selected_notifications ) ) ?> />
														<label for="gf_zarinpal_notif_1_<?php echo $notification['id'] ?>" class="inline" for="gf_zarinpal_selected_notifications"><?php echo $notification['name']; ?></label>
													</li>
												<?php } ?>
											</td>
										</tr>
										...
										<tr id="gf_zarinpal_notifications" <?php echo !isset($form["notifications"]) ? "style='display:none;'" : "" ?>>
											...
											<td>
												...
												foreach ( $notifications as $notification ) {	?>
													<li class="gf_zarinpal_notification">
														<input id="gf_zarinpal_notif_2_<?php echo $notification['id'] ?>" name="gf_zarinpal_notif_2[]" type="checkbox" class="notification_checkbox" value="<?php echo $notification['id'] ?>" <?php checked( true, in_array( $notification['id'], $selected_notifications ) ) ?> />
														<label for="gf_zarinpal_notif_2_<?php echo $notification['id'] ?>" class="inline" for="gf_zarinpal_selected_notifications"><?php echo $notification['name']; ?></label>
													</li>
												<?php } ?>
											</td>
										</tr>	
										...
										<tr id="gf_zarinpal_notifications" <?php echo !isset($form["notifications"]) ? "style='display:none;'" : "" ?>>
											...
											<td>
												...
												foreach ( $notifications as $notification ) {	?>
													<li class="gf_zarinpal_notification">
														<input id="gf_zarinpal_notif_3_<?php echo $notification['id'] ?>" name="gf_zarinpal_notif_3[]" type="checkbox" class="notification_checkbox" value="<?php echo $notification['id'] ?>" <?php checked( true, in_array( $notification['id'], $selected_notifications ) ) ?> />
														<label for="gf_zarinpal_notif_3_<?php echo $notification['id'] ?>" class="inline" for="gf_zarinpal_selected_notifications"><?php echo $notification['name']; ?></label>
													</li>
												<?php } ?>
											</td>
										</tr>	
										...
										<tr id="gf_zarinpal_notifications" <?php echo !isset($form["notifications"]) ? "style='display:none;'" : "" ?>>
											...
											<td>
												...
												foreach ( $notifications as $notification ) {	?>
													<li class="gf_zarinpal_notification">
														<input id="gf_zarinpal_notif_4_<?php echo $notification['id'] ?>" name="gf_zarinpal_notif_4[]" type="checkbox" class="notification_checkbox" value="<?php echo $notification['id'] ?>" <?php checked( true, in_array( $notification['id'], $selected_notifications ) ) ?> />
														<label for="gf_zarinpal_notif_4_<?php echo $notification['id'] ?>" class="inline" for="gf_zarinpal_selected_notifications"><?php echo $notification['name']; ?></label>
													</li>
												<?php } ?>
											</td>
										</tr>	
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="radio" name="gf_zarinpal_shaparak" id="gf_zarinpal_shaparak_raygan" value="raygan" <?php echo rgar($config['meta'], 'shaparak') != "sadt" ? "checked" : "" ?>/>
                                                <label class="inline" for="gf_zarinpal_shaparak_raygan"><?php _e("رایگان شود", "gravityformszarinpal"); ?></label>

                                                <input type="radio" name="gf_zarinpal_shaparak" id="gf_zarinpal_shaparak_sadt" value="sadt" <?php echo rgar($config['meta'], 'shaparak') == "sadt" ? "checked" : "" ?>/>
                                                <label class="inline" for="gf_zarinpal_shaparak_sadt"><?php echo $minprice . __(" شود ", "gravityformszarinpal"); ?></label>
                                                <span class="description"><?php _e('(قابل استفاده برای زمانیکه منطق شرطی درگاه فعال نباشد)', 'gravityformszarinpal'); ?></span>
                                        ...
                                        <tr>
											...
											<td>
												...
												<label for="gf_zarinpal_conditional_enable"><?php _e("فعالسازی این درگاه اگر شرط زیر برقرار باشد :", "gravityformszarinpal"); ?></label><br/>
														...
														<td>
															<div id="gf_zarinpal_conditional_container" <?php echo !rgar($config['meta'], 'zarinpal_conditional_enabled') ? "style='display:none'" : ""?>>
																<div id="gf_zarinpal_conditional_fields" style="display:none">
																	<select id="gf_zarinpal_conditional_field_id" name="gf_zarinpal_conditional_field_id" class="optin_select" onchange='jQuery("#gf_zarinpal_conditional_value_container").html(GetFieldValues(jQuery(this).val(), "", 20));'></select>
																...
																<div id="gf_zarinpal_conditional_message" style="display:none;background-color:#FFDFDF; margin-top:4px; margin-bottom:6px;padding:18px; border:1px dotted #C89797;">
										...
										<tr>
											<td>
												<input type="submit" class="button-primary gfbutton"  name="gf_zarinpal_submit" value="<?php _e("ذخیره", "gravityformszarinpal"); ?>"/>
											</td>
										</tr>
									</tbody>
								</table>
	...
	public static function Request($confirmation, $form, $entry, $ajax){
		...
		if ( ! $valid_checker ) {
			...
			$Description = __('جهت بررسی تنظیمات صحیح بودن تنظیمات درگاه گراویتی فرم زرین پال', 'gravityformszarinpal');
