...
register_activation_hook( __FILE__, array( 'GFPersian_Gateway_ZarinPal', "add_permissions" ) );
add_action( 'init', array( 'GFPersian_Gateway_ZarinPal', 'init' ) );

require_once( 'database.php' );
require_once( 'chart.php' );

class GFPersian_Gateway_ZarinPal {
	...
	private static function setup() {
		if ( get_option( "gf_zarinpal_version" ) != self::$version ) {
			GFPersian_DB_ZarinPal::update_table();
	...
	public static function uninstall() {
		...
		GFPersian_DB_ZarinPal::drop_tables();
	...
	public static function get_config_by_entry( $entry ) {
		...
		$feed    = ! empty( $feed_id ) ? GFPersian_DB_ZarinPal::get_feed( $feed_id ) : '';
	...
	public static function get_active_config( $form ) {
		...
		$configs = GFPersian_DB_ZarinPal::get_feed_by_form( $form["id"], true );
	...
	public static function zarinpal_page() {
		...
		if ( $view == "edit" ) {
			...
			GFPersian_Chart_ZarinPal::stats_page();
		} else {
			self::list_page( '' );
		}
	}
	...
	private static function list_page( $arg ) {
		...
		if ( rgpost( 'action' ) == "delete" ) {
			...
			GFPersian_DB_ZarinPal::delete_feed( $id );
			?>
			...
			if ( is_array( $selected_feeds ) ) {
				foreach ( $selected_feeds as $feed_id ) {
					GFPersian_DB_ZarinPal::delete_feed( $feed_id );
				}
			}
        ...
        <div class="wrap">
            ...
            <form id="confirmation_list_form" method="post">
                ...
                <table class="wp-list-table widefat fixed striped toplevel_page_gf_edit_forms" cellspacing="0">
					...
					<?php
					if ( $arg != 'per-form' ) {
						$settings = GFPersian_DB_ZarinPal::get_feeds();
					} else {
						$settings = GFPersian_DB_ZarinPal::get_feed_by_form( rgget( 'id' ), false );
					}
	...
	public static function update_feed_active() {
		...
		$feed = GFPersian_DB_ZarinPal::get_feed( $id );
		GFPersian_DB_ZarinPal::update_feed( $id, $feed["form_id"], $_POST["is_active"], $feed["meta"] );
	}
	...
	private static function config_page() {
        ...
        <div class="wrap gforms_edit_form gf_browser_gecko">
			...
			$config    = empty( $id ) ? array(
				"meta"      => array(),
				"is_active" => true
			) : GFPersian_DB_ZarinPal::get_feed( $id );
			$get_feeds = GFPersian_DB_ZarinPal::get_feeds();
			$form_name = '';
			...
			if ( ! rgempty( "gf_zarinpal_submit" ) ) {
				...
				$id = GFPersian_DB_ZarinPal::update_feed( $id, $config["form_id"], $config["is_active"], $config["meta"] );
				if ( ! headers_sent() ) {
                ...
                <div id="gf_form_toolbar">
                    <ul id="gf_form_toolbar_links">
                        ...
                        <li class="gf_form_switcher">
							...
							<?php
							$feeds = GFPersian_DB_ZarinPal::get_feeds();
                                    ...
                                    <tr style="<?php echo rgget( 'id' ) || rgget( 'fid' ) ? 'display:none !important' : ''; ?>">
                                        ...
                                        <td>
                                            <select id="gf_zarinpal_form" name="gf_zarinpal_form"
												...
												<?php
												$available_forms = GFPersian_DB_ZarinPal::get_available_forms();
